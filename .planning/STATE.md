...
- applySqlitePragmas() applies WAL + busy_timeout=5000 each wrapped (a PRAGMA failure warns, never crashes boot); called from instrumentation register() BEFORE seed/startWatcher. connection_limit=1 lives on DATABASE_URL (.env.example + DEPLOY.md, placeholder paths only), not in code.
